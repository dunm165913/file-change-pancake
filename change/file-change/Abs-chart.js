...
class AbstractChart extends Component {
  ...
  renderHorizontalLabels = config => {
    ...
    const decimalPlaces = this.props.chartConfig.decimalPlaces === undefined ? 1 : this.props.chartConfig.decimalPlaces
  ...
  renderVerticalLabels = config => {
    ...
    const realLabel = labels.filter(l => { return l != '' }).length
    ...
    return labels.map((label, i) => {
      ...
      return (
        <Text
          origin={`${x}, ${y}`}
          rotation={realLabel > 7 ? realLabel > 12 ? 90 : -45 : 0}
          key={Math.random()}
          x={x}
          y={y}
          fontSize={fontSize}
    ...
    return [...new Array(data.length)].map((_, i) => {
      return (
        <Line
          ...
          y2={height - height / 4 + paddingTop}
          stroke={this.props.selected == i ? 'red' : this.props.chartConfig.color(0.2)}
          strokeDasharray="5, 10"
          strokeWidth={1}
        />
      )
