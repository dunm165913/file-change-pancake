...
function htmlToElement(rawHtml, opts, done) {
  function domToElement(dom, parent) {
    ...
    return dom.map((node, index, list) => {
      ...
      if (node.type == 'text') {
        return (
          <Text key={index} style={[parent ? opts.styles[parent.name] : null, { borderRadius: 10, overflow: 'hidden'}]}>
            { parent && parent.name == 'span' ? " ": null}
      ...
      if (node.type == 'tag') {
        ...
        return (
          <Text key={index} onPress={linkPressHandler} style={{padding: 10}}>
